...
| 1 | Foundation & Platform Kernel | 🟡 **Substantially complete** | Builds, runs, verified manually. The note here said "integration tests unrun" for nineteen phases; they run on every push against PostgreSQL 17, and what is unrun is running them on *this machine* (§4) **Reviewed 2026-09-15 against every acceptance criterion** (PROJECT_PLAN.md). Remaining: CI has never been shown to fail on a planted secret (it scans every pushed commit), and `docker compose up` has never been run on a developer machine. |
| 2 | Identity & Authentication | 🟡 **Feature-complete** | All planned tasks built: authentication, password management and reset, user administration, `/me`, JWKS, bootstrap seeder. 116 unit tests. **Corrected on re-reading:** this row said the integration tests were unrun and the permission strings unenforced. Both stopped being true in Phase 4 — the tests run in CI on every push, and `PermissionPolicyProvider` turns the attribute into a requirement that `PermissionAuthorizationHandler` answers. See #5 and #3. **Reviewed 2026-09-15 against every acceptance criterion** (PROJECT_PLAN.md). Remaining: **a user cannot revoke one of their own sessions** — listing exists, revoking does not; coverage 82.2% against 85%, the gap being the handlers (Application 47%). |
| 3 | Organization | 🟡 **Core complete** | Unit hierarchy with materialized path, arbitrary depth, atomic moves, cycle prevention, bilingual names, employees, positions, company. 48 unit tests. **Corrected on re-reading:** this row said the position and company endpoints and the integration tests were outstanding. All three exist — `GET`/`POST /organization/company` and `/positions` with their edit endpoints, and two integration suites against a real database. The employee extension bag (§7.2.2) arrived later, as #14. **Reviewed 2026-09-15 against every acceptance criterion** (PROJECT_PLAN.md). Remaining: the 5,000-node tree query has never been timed (#74); coverage 67.8% against 80% (Application 12%). |
| 4 | Authorization & RBAC | 🟡 **Core complete** | RBAC with organizational scope, enforcement wired, anti-escalation, version-stamped cache, application registry, permission declaration. Scope filter applied at the data layer for employee search. 63 unit tests. **Reviewed 2026-09-15 against every acceptance criterion** (PROJECT_PLAN.md). Every functional criterion is met. **The refusal to grant an unheld permission had no test** and now has three. Remaining: coverage 76.0% against 85% (Application 28%). |
| 5 | Security Hardening | 🟡 **Core complete** | Per-endpoint rate limits, TOTP two-factor with recovery codes, AES-256-GCM secret protection, step-up authentication enforced on six privileged endpoints, security event log with bounded search. 56 unit tests, 3 new architecture tests. **Reviewed 2026-09-15 against every acceptance criterion** (PROJECT_PLAN.md). Three rows of its actual-result table were stale — database verification, the 429 over HTTP and the administrative MFA reset had all been done since. Coverage **87.6%, meeting its 85%**. Remaining: the planted-secret demonstration, detection thresholds (#55), trusted devices (deferred by decision). |
| 6 | Audit | 🟡 **Core complete** | Append-only trail, monthly range partitioning with a maintenance job, redaction before storage, internal and external ingestion, bounded search, privileges revoked to INSERT+SELECT at the database. **Wired into 15 state-changing handlers across Identity, Organization, Authorization and Security**, through a neutral kernel seam so no module references Audit. 50 unit tests + 3 architecture tests. **Reviewed 2026-09-15 against every acceptance criterion** (PROJECT_PLAN.md). The append-only integration tests it listed as unwritten exist and run. Remaining: asynchronous export and partition-detach retention are not built; an external application ingesting events has no end-to-end test; the 10-million-row search budget is unmeasured; coverage 79.6% against 80%. A rolled-back change can leave an audit entry, because the recorder commits on its own context by design. |
...
**11 of 22 phases complete** (phase 0 and ten others), **10 in progress**, **1 not started** — counted from the table above rather than remembered, and now checked by `scripts/check-status-consistency.py` on every push.
...
| 3 | ~~`RequirePermissionAttribute` declares intent but does not enforce~~ | — | ✅ **Resolved by evidence rather than by work, and stale since Phase 4.** `PermissionPolicyProvider` turns the attribute's `permission:` policy name into a requirement and `PermissionAuthorizationHandler` answers it, both in the Authorization module and both exercised by the authorization matrix on every push. The entry described Phase 1 and was never read again — which is the third such row found today, and the reason a register is worth re-reading rather than only appending to. |
...
| 6 | Breach screening implemented but **disabled by default** | Low — **it is the owner's call; what was wrong with it is fixed** | 🟡 Still off by default, and that stays the owner's decision: it makes an outbound call to a third party, and a Platform that starts doing that the moment it boots has made that decision for them. **But re-reading it found something else.** The checker reached `api.pwnedpasswords.com` directly — no allow-list, no address check, no entry in the call log — which is the same defect as #33/#45, found a second time. It now asks `IOutboundGateway` to approve the host and records the attempt, so enabling screening also means allow-listing the host, which is the visibility deny-by-default is for. **Nothing about the password reaches the log**: not the password, not its hash, not the prefix that would be sent — a row hinting somebody's chosen password was in a breach corpus is the last thing that should be browsable. 4 unit tests, verified against a deliberately decorative check. |
...
| 20 | TOTP is not phishing-resistant | Medium | A convincing fake login page can collect and replay a code within thirty seconds. WebAuthn is the answer, and the company does not yet have the hardware for it. **This row used to say WebAuthn "is kept as an extension point", and that was not true** — there is no second-factor abstraction anywhere: `MfaEnrolment` stores a shared secret and has no notion of which method produced it, and the endpoints, DTOs and handlers name TOTP directly. Adding WebAuthn means a method discriminator, a different kind of stored material (a credential id, a public key and a signature counter — none of them a secret needing the protection this one gets) and its own endpoints. Not building that seam now is still the right call, because a seam designed against a method nobody can use yet would be shaped by this method anyway; but "extension point" reads as "write a provider and plug it in", and somebody planning a phishing-resistant rollout off that sentence would be planning against nothing. Accepted with its real cost written down. |
...
| 22 | ~~Argon2id parameters are defaults, not measured~~ | — | ✅ **Resolved, by making the measurement happen rather than by choosing a number.** It stayed open for twenty phases and the reason was never laziness: the answer depends on hardware nobody had chosen. It still does. What was missing was somewhere for the measurement to occur — the parameters were a guess until something running said what they cost, and no deployment could have told you without somebody signing in with a stopwatch. The Platform now hashes once at startup and writes the figure to the log, warning below 100 ms because **cheap for the Platform is cheap for somebody working through a stolen password table**, and more mildly above a second. **It reports and does not tune**: hashing cost is a security parameter, and a Platform that raised its own would change every sign-in on a schedule nobody chose, with no record of the old value — and a machine busy during startup would pick a number wrong for every hour after. Two hashes, the first discarded, because it pays for the JIT and the first sixty-four-mebibyte allocation that a real sign-in does not. 4 unit tests, one of which asserts the production parameters clear the floor on whatever hardware the suite runs on. `docs/security/authentication.md` §7. |
...
| 28 | Workflow has no callback for business-conditional routing | Medium — **and the escape it rests on now works properly and is tested** | 🟡 Still no callback, which stays right: the second escape in §16.3 is an interface the Platform would call into a business system, and building one before a business system exists to answer is designing against an imagined caller. **The first escape is the one that matters, and checking it found it half-built.** `AssigneeStrategy.SuppliedByCaller` resolved correctly and **nothing tested it** — the answer to every "how do I route on business data" question, and the reason the no-business-rules boundary is defensible at all, with nothing to notice it breaking. Worse, publication accepted it on **any** step, while the engine deliberately enters every step after the first with an empty supplied list. Such a definition published happily and stalled when an instance reached that step, telling the person who filed the request weeks earlier to "check that the role, position or manager it names still exists" — it names none. Publication now refuses it and names the step, and starting an instance whose first step wants a list and did not get one says **that**, instead of sending somebody to read the org chart for a field they forgot to send. 4 tests, verified by disabling the check and watching exactly the two refusal tests fail. |
...
| 33 | ~~The email provider is a direct SMTP adapter~~ | — | ✅ **Resolved.** The email channel was the one outbound call in the Platform that passed no door at all: it opened a socket to a mail server directly, with no allow-list, no entry in the call log, and no way for an operator to discover the Platform had been failing to send anything for a day. Both entries said why it stayed open — **SMTP is not HTTP**, so the connector cannot carry it — and both were right about the connector and wrong to stop there. **An allow-list is a question about a host name and a call log is a row**; neither cares what happens on the socket afterwards. So Integrations' Contracts gained one narrow surface, `IOutboundGateway`, offering exactly those two things to a channel that speaks something else. The mail host now passes the same allow-list and the same private-address checks as every other outbound call, and every attempt lands in the shared call log under the provider code `smtp` — which puts the mail server in the health view beside the HTTP providers. A refused host is **permanent**, because retrying a configuration fact for half an hour delays every message behind it. **The host is recorded, never the recipient**: the call log is read by administrators and exported, and this would be the one place in the Platform where a list of who was emailed existed. Deliberately **no second retry policy** — the dispatcher already retries with backoff and gives up after five attempts, and a breaker here as well would be two policies over one failure. 4 unit tests, and the one that matters refuses at the door and then insists nothing was sent: a check wired in and never consulted looks exactly like one that works. Verified against a deliberately decorative check. `docs/notifications/README.md` §3. |
...
| 36 | ~~The object-storage path is not tested against a real bucket~~ | — | ✅ **Resolved.** CI now runs MinIO — the same image and the same command as `docker-compose.yml`, so what CI exercises is what a developer runs — and 9 integration tests drive the S3 provider against a real bucket it creates and removes. **The assertion the suite exists for: the same address without a signature is refused**, checked alongside a signed fetch that succeeds so it is a test of the signature rather than of a bucket name that was wrong all along. A public bucket would make every access rule in the Documents module decorative, since anybody who learned a key could fetch the file without reaching the Platform — and the key is in the URL of every download it has ever issued. Also covered, and uncheckable against the local provider: that a pre-signed URL **expires**, that it carries the file name and forces a download, that a name carrying a quotation mark cannot break out of the header, and that a missing object reads as nothing rather than throwing. A step rather than a service container, because MinIO needs its `server /data` arguments and GitHub's services syntax has nowhere to put them. **It found a production defect on its first run**: the SDK's `GetPreSignedUrlRequest.Protocol` defaults to HTTPS whatever `ServiceURL` says, so a deployment reaching its object storage over plain HTTP issued download links that could not be fetched at all — while every API call worked, because those honour the endpoint. Only the links handed to browsers were wrong, which is the half no unit test touches, and is precisely the assurance gap this entry was about. The scheme now comes from the configured endpoint, falling back to HTTPS when none is named. |
| 37 | A ZIP is accepted on the strength of its extension | Low | The bytes prove it is an archive; which member of the ZIP family it is comes from the file name, because telling a `.docx` from an `.xlsx` means opening the archive. The security question is answered by the content and the cosmetic one by the name, so the worst outcome is a spreadsheet labelled as a document. **Refined after re-reading the code:** an archive whose extension is not one the Platform recognises is now *refused* rather than stored as a generic archive, so the name decides which member of the ZIP family it is and cannot smuggle in an unknown one. |
...
| 45 | ~~The email channel has not been moved onto the integration layer~~ | — | ✅ **Resolved.** The email channel was the one outbound call in the Platform that passed no door at all: it opened a socket to a mail server directly, with no allow-list, no entry in the call log, and no way for an operator to discover the Platform had been failing to send anything for a day. Both entries said why it stayed open — **SMTP is not HTTP**, so the connector cannot carry it — and both were right about the connector and wrong to stop there. **An allow-list is a question about a host name and a call log is a row**; neither cares what happens on the socket afterwards. So Integrations' Contracts gained one narrow surface, `IOutboundGateway`, offering exactly those two things to a channel that speaks something else. The mail host now passes the same allow-list and the same private-address checks as every other outbound call, and every attempt lands in the shared call log under the provider code `smtp` — which puts the mail server in the health view beside the HTTP providers. A refused host is **permanent**, because retrying a configuration fact for half an hour delays every message behind it. **The host is recorded, never the recipient**: the call log is read by administrators and exported, and this would be the one place in the Platform where a list of who was emailed existed. Deliberately **no second retry policy** — the dispatcher already retries with backoff and gives up after five attempts, and a breaker here as well would be two policies over one failure. 4 unit tests, and the one that matters refuses at the door and then insists nothing was sent: a check wired in and never consulted looks exactly like one that works. Verified against a deliberately decorative check. `docs/notifications/README.md` §3. |
| 80 | ~~Nothing stopped a fourth outbound call going round the door~~ | — | ✅ **Resolved the day the second one was found.** The email channel (#33, #45) and breached-password screening (#6) both reached third parties directly, both were written before the integration layer existed, and both were found by reading rather than by anything failing. **Two is a class, and a class is closed by a guard.** `OutboundCoverageTests` fails the build on an outbound client that is not in a reviewed set carrying its reason — the same shape as the reviewed anonymous-endpoint surface and the step-up set. It also refuses a reviewed entry whose file no longer builds a client, because an exception outlives the code it excuses and would stand open for whatever is written there next; and a third assertion fails if the scan stops seeing the source at all, since a rule expressed as "nothing matched" passes for ever the moment the matching breaks. It cannot check that a listed client actually asks the door — that is a call at the top of a method, not a shape a scan can see — so each has its own test refusing at the door and insisting nothing was sent. |
| 81 | ~~Nothing checked that a documented endpoint exists, and its own exception list had no expiry~~ | — | ✅ **Resolved.** Documentation drift is this project's most common defect by a wide margin: in one afternoon four documents were found describing a Platform that had stopped existing phases earlier — endpoints listed as missing that had shipped, a table of six unbuilt things of which five were built, and **an operational warning telling readers not to deploy** because of a limitation fixed twenty phases before. Every one was caught by a person reading; none by the build, because nothing checked. `scripts/check-doc-endpoints.py` now runs in CI over all 61 endpoint citations in the documentation and asks the contract whether each really exists, matching templated segments so a real example with a concrete id still checks. It found one on its first run: ARCHITECTURE.md listed `POST /api/v1/audit/exports` flatly beside two endpoints that exist — now marked as designed-and-not-built at the point it is named, and allowed with its reason, since that document has standing to describe the intended architecture. **What it cannot check is the opposite drift** — a document claiming something is *not* built when it is, which is a claim about absence and has no shape a scan can match. That stays a human job, and is why the register is worth re-reading rather than only appending to. **Strengthened after the fact:** the checker shipped with an `ALLOWED_ABSENT` list and no check that its entries were still absent — the exact omission the outbound guard has a second assertion for, made in the guard written the same day. An exception outlives the code it excuses: once the asynchronous export is built, that entry stops meaning "documented on purpose, not yet real" and starts meaning "this one route is never checked", for whatever is written at that path next. It now fails on an entry that names something the contract has, verified by adding the route to a copy of the contract and watching it refuse. |
| 82 | ~~Nothing enforced snake_case column naming~~ | — | ✅ **Resolved.** The Phase 1 defect was `ApplySnakeCaseNames()` written and never called: columns generated as `Username` while hand-written partial index filters referred to `username`, so **the indexes would have failed on the first deployment** and nothing would have said why until somebody read the generated SQL. It was fixed by adding one line to one `OnModelCreating` — exactly the kind of fix that does not stay fixed, since the twelfth module has to remember the same line. `adding-a-module.md` called it "not optional, and not cosmetic", and **that sentence was the entire enforcement** — the same arrangement `[FromServices]` had until this morning. `ColumnNamingTests` now builds every module's model through its own design-time factory (no database needed: EF resolves the model from `OnModelCreating` alone) and fails on any table or column that is not snake_case. Verified by commenting the line out of one context and watching it name the columns. A third assertion counts the models, because both others say "nothing was wrong" — which is what a test says when it has stopped looking. That count found **my own** wrong expectation first: eleven contexts, not twelve, because the Operations module owns no data at all. |
| 90 | ~~Ten of eleven unit test projects never ran in CI~~ | — | ✅ **Resolved, and the worst finding of the phase review.** The CI step called "Unit tests" named one project, the kernel's. The ten module projects beside it were in the solution, compiled by every build, and never executed: **101 unit tests ran in CI out of 812 written.** Every module's domain rules — password hashing, lockout, the step-up refusal, workflow publication rules, subscription event types — were green only on whichever machine last ran them, and a regression in any of them would have shipped with the pipeline green. This session's own reports of CI verifying module unit tests were, for the same reason, reports of local runs. Found by downloading the test-results artifact to read coverage and counting what was in it: 101, 66 and 201. The step now discovers every unit test project by glob, so a new module's tests run the day they exist, and fails if the glob finds fewer than eleven, because one that stops matching runs nothing and passes. **Its first run in CI failed, and was right to**: `CheapParametersAreWarnedAbout` hashed with deliberately cheap Argon2 parameters and expected a warning, letting the machine decide — on the runner those parameters took longer than the 100ms floor, so nothing was logged. It had passed on every laptop. The report now separates deciding from measuring and the tests give it fixed costs; the loop also runs every project before failing, since the first run stopped at Identity and left five projects unrun. |
| 99 | ~~The Platform had no way in that could not be phished~~ | — | ✅ **Resolved 2026-09-17.** Passkeys: signing in with the device’s own fingerprint, face or passcode. **No biometric reaches the Platform** — the sensor unlocks a private key the device never surrenders, and the Platform stores only the public half, so a database taken whole yields nothing anybody can sign in with. It is the first credential here that **cannot be phished**: the browser binds it to this domain, so a convincing copy of the sign-in page on another one cannot ask for it. **Sign-in takes no username**, because an endpoint that took one and answered differently depending on whether that account had a passkey would be the account enumeration this Platform refuses everywhere else. **Adding one needs the password** — a passkey outlives a password change, so adding one is what somebody holding a stolen session would do to keep their way in — but deliberately not step-up, which would mean only people who already had an authenticator app could ever set up a fingerprint. **A failed attempt does not count towards lockout**: a passkey cannot be guessed, and if failures counted, somebody who had once seen a credential identifier could lock a person out of their own account from across the internet. **The WebAuthn checks are written out rather than taken from a library**, because the subset a platform authenticator needs is small and every line of it is a rule worth reading; the CBOR reader is the runtime’s. They are tested against **a registration and a sign-in captured from Chrome’s own implementation**, since a fixture written by the same understanding that wrote the verifier proves only that the understanding is self-consistent — and **that test found a real defect on its first run**: coordinates that are not a point on the curve parse as CBOR perfectly well, so an invalid key was stored and then threw `PlatformNotSupportedException` on every sign-in attempt for ever. Keys are now built where they are accepted, so “we stored it” and “we can verify with it” are the same statement. Sixteen unit tests, most of them refusals, one per check; an end-to-end test registers a device with Chrome’s virtual authenticator, signs out and signs back in with it; a second proves an authenticator that cannot verify a person is refused. The relying party and its origins are checked at startup and **every problem is named**, because the symptom of a mismatch is the browser declining silently and nothing reaching the server at all. |
| 98 | ~~The dashboard said the right things in the wrong shape~~ | — | ✅ **Resolved 2026-09-17.** Once it had something to say (#95) the layout was what held it back: four bands of identical weight stacked down the middle of a wide screen, so the page had no first thing to look at and took a scroll to finish, with roughly a third of it inked. **The biggest type on the page was the least useful thing on it** — the four counts set at three times the body size, while the sentence saying event delivery had stopped was set at the body size. A dashboard is two different things: what needs a decision is prose and is read, what the Platform holds is reference and is looked up. The decisions now take two thirds of the width and the reference stands beside them as a list of facts rather than four billboards, which halves the height and inverts the hierarchy back. **Each condition now names the screen it is dealt with on**, in brackets after the sentence like the "(Required)" beside a field label: the row used to end in four hundred pixels of nothing, and nothing was what it told the reader about where the link went. Aligning those names to the end of the row was tried first and left a differently sized hole in every row, because the sentences are of very different lengths. The read-at time moved out of the description into a field of its own on the heading, since gluing a fact to a description makes one run-on sentence of two unrelated statements. Checked in both languages and on a phone, where the columns fold back into the order they are written in; axe reports no violation in either. |
| 97 | ~~The portal could not be installed, and the image would not have carried it~~ | — | ✅ **Resolved 2026-09-17.** The Platform is now an installable application: a manifest per language, its own icons, a service worker and an honest page when the network is gone. **The manifest is served per locale** because the name on a home screen cannot be translated afterwards — one static manifest would have meant half the company installing an application named in a language they did not choose — while `id` stays the same in both, so installing from the English pages updates the application already there rather than leaving a second copy. **The worker caches almost nothing, deliberately.** Everything this application serves is rendered for one signed-in person, and a worker cache survives sign-out, survives a different person signing in on the same machine, and answers from disk before the network is consulted: it holds build output whose filenames carry a content hash, the icons and the offline page, and nothing else. Anything under `/api` it declines to touch at all. The decision is a pure function so it can be tested without booting a worker, and **ten tests cover it, each verified against a deliberately broken policy** — the BFF guard removed, pages served from the cache, a prefix widened from `/icons/` to `/icons`, and the fallback made to store the page it fetched. The offline page was proved by stopping the server with a browser attached, not by emulating it: Playwright’s `setOffline` did not block the request, and the 200 it returned was the redirect to sign-in. **`public/` was named in the frontend Dockerfile’s comments and never copied**, so every one of these files would have been missing from the deployed image while working in development, in `next start` and in the end-to-end suite, all three of which read them from the working tree. **Nothing built the portal image in CI at all** — only the API’s — so the omission had nowhere to surface; the container job now builds it and runs it, and fails if the running image does not serve the sign-in page, the manifest, the worker, the offline page and all four icons. The icons are drawn by a committed script rather than by hand, as a wordmark rather than a picture, for the reason §9.5 gives: a glyph invented for this Platform would mean nothing to anybody until they had learned it. |
| 96 | ~~Every screen was drawn well and assembled carelessly~~ | — | ✅ **Resolved 2026-09-16, by looking at the deployed screens rather than at the components.** The parts were right and the assembly was not. **Every list screen put its search box in a container told to take every pixel going**, so a box holding one username was eleven hundred pixels wide on a wide monitor; a control’s width is a claim about what goes in it, and that one was false on fourteen screens. **Sixteen inputs across twelve screens were hand-rolled** at the old shared height, so when the shared control came down they stayed, and a toolbar with a nine and a ten in it reads as assembled rather than designed. **The table was drawn as a box with a tinted header band** — a spreadsheet pasted into the page rather than part of it; it is now a rule under the headings and hairlines between rows, with nothing around it. **Row actions ran together**: in Arabic the words are shorter and set tighter, and the roles screen showed three across one cell where the middle one is a noun, so "edit permissions deactivate" read as a single instruction; they are separated by a hairline now, drawn by the table on every action after the first. **The first attempt at it produced no separator anywhere**: it walked React’s children, and every screen wraps its actions in a permission guard, which is one child. The guard renders to nothing, so the rule is selected in the document instead, where the actions really are siblings — and four screens that wrapped their actions in a container duplicating the table’s own, with a tighter gap, lost it. **The empty state was a full-width bordered block with its text on the start edge** — the shape of an error banner, for "no results"; it is centred between two rules. **The status column was a column of boxes before it was a column of states**: the one state nobody has to act on now carries no box at all. **The sort direction read as part of the heading** — a column called "Name Ascending" until you worked out that it was not — and is bracketed like the "(Required)" beside a field label. **The sign-in area had no way to change the language**, anywhere: the switch lived in the application shell, which is behind sign-in, so somebody sent a link to the Arabic sign-in page who reads English had no way out of the one page everybody reaches. **The audit screen opened as two blank date boxes** in a format the browser chooses and the application cannot set, and refused to search until they were filled; it opens on the last seven days, written into the boxes where the reader can see and change them, which keeps the reason for the original refusal — no range is ever implicit — without the blank screen. `design-constraints.test.ts` now fails the build on a hand-rolled control that is not the shared height; **its first run found a sixteenth input the sweep had missed**, because its class list was in a different order. The audit prompt was corrected with it: the screen still said "choose a date range" while the range sat filled in above it. Five tests over the real table, verified against a screen that wraps its actions and against the rule being deleted. Checked in both languages against the preview, with axe reporting no violation on any of them. |
| 95 | ~~The dashboard said the same thing every day, whatever the Platform was doing~~ | — | ✅ **Resolved 2026-09-16.** It showed four counts and four setup instructions. The instructions were static: they never disappeared once followed, so a Platform configured a year earlier still opened on "build the organizational structure". The counts described the size of the Platform, not its state — nothing on the screen would change if event delivery were three days behind, a background job had been failing every run, a webhook subscription had been suspended, or a task had been waiting on the reader for a week. **The one screen everybody opens first could not report a single thing going wrong.** It now leads with the conditions that are true now, each disappearing when it stops being true, and the setup steps are among them; then what is waiting on the reader personally, then the counts, then the outbox, the jobs and the subscriptions. Eleven reads, in parallel, **issued only where the caller holds the permission** — an ordinary employee would otherwise generate half a dozen refusals in the security log on every load, which is noise in the one place noise is expensive. **A reading that was not taken produces no sentence**: null is not zero, and reporting health from questions the caller was not allowed to ask would be a lie that looks like health. The conditions are a pure function with 13 unit tests, each verified against a broken input. Arabic counts are ICU plurals across all six categories, and a new test formats **every message in both catalogues at eleven numbers** — a malformed ICU message does not throw, it renders its own key, so nothing would have shown the error until the day the condition first came true; verified by planting an unbalanced brace and a number dropped into a fixed phrase, which it caught. An end-to-end test asserts the four bands render and that **nothing reads as unavailable for the administrator**, which is what would fail if a permission name here stopped matching the Platform’s catalogue: every read skipped, the whole Platform reported unreadable, and not one error anywhere. |
| 91 | A user cannot revoke one of their own sessions | **Medium** | Phase 2 acceptance criterion, never built. `GET /me/sessions` lists them; nothing ends one. Somebody who receives the new notice that two-factor authentication was turned on, or who simply suspects a stolen session, can end only the session they are signed in with. Buildable without any decision. |
| 94 | ~~The portal's own dependencies were never audited, and the framework had three critical remote-code-execution advisories~~ | — | ✅ **Resolved 2026-09-16, and found by making the repository public.** CI audited the .NET packages and scanned the container image; **nothing had ever asked npm**. GitHub's scanner, switched on with the repository, reported **43 open advisories in the portal's dependencies within minutes**: 4 critical and 18 high, all npm. Among them **three critical remote-code-execution flaws in Next.js itself** — the image optimiser and the React flight protocol — plus middleware and proxy bypasses, server-side request forgery and denial of service, in 15.5.4, the version **serving the portal on the internet**. Upgraded to 15.5.25 with next-intl 4.14.5, vitest to 3.2.7 (its own critical was an arbitrary file read through the test UI) and postcss to 8.5.28; `npm audit fix` cleared the rest. **Critical is now zero.** One high remains, postcss bundled inside Next, reachable only at build time and fixable only by the major upgrade to Next 16 — recorded rather than silently gated around. CI now audits the portal's dependencies on every push: critical in a runtime dependency fails the build, high is reported. GitHub's push protection, secret scanning and Dependabot updates are on. A full history scan of every commit found no real credential: the single secret-scanning alert was a deliberately fake Slack token in the test that checks the Platform's own secret detector, resolved as a false positive. |
| 93 | ~~The portal read as assembled from a template, and three defects hid behind that~~ | — | ✅ **Resolved 2026-09-15, by looking at it.** The design was restrained on paper and generic on screen: a small card centred on a grey page for sign-in, four equal statistic tiles on the dashboard, rounded pills in a flat fifteen-item sidebar, and status tags in green, amber and red. Now white and blue only, **state included** — intensity along one blue instead of three hues — near-square corners, a deep-blue top bar that says which system this is, a grouped sidebar, a split sign-in page, a one-panel dashboard, and every symbol standing in for a word replaced by the word: empty cells, the sort triangle, workflow arrows, separators, and twelve em dashes per language in the catalogue. Checked through a development-only preview page that renders the real components with sample data and returns 404 in production (verified against `next start`), captured with Playwright before and after. **Looking found what reading had not:** (1) **every dialog in the portal, production included, opened pinned to the top corner of the screen** — Tailwind's reset zeroes the margin a modal dialog is centred by; (2) **the IBM Plex faces the tokens named had never been loaded**, so every browser rendered Segoe UI or Tahoma; (3) the skip link announced the word for “search”. `design-constraints.test.ts` now measures every colour token's hue, and fails on a symbol in rendered code or either catalogue and on large radii — each verified by planting the violation. |
| 92 | Coverage is below target in four of five audited phases | Low | First measured 2026-09-15 by merging local unit coverage with CI's integration coverage: Identity 82.2% (target 85), Organization 67.8% (80), Authorization 76.0% (85), Audit 79.6% (80); Security 87.6% meets 85. The shortfall is consistently the Application layer — handlers exercised mostly through a few integration paths — while Domain sits at 69–99%. Coverage is collected on every push and still not reported or enforced. |
| 89 | ~~A notification that exposes account takeover could never be sent~~ | — | ✅ **Resolved.** “Two-factor authentication was turned on for your account — if this was not you, tell your administrator” was seeded in both languages and editable in the portal, and nothing could deliver it: **the Security module published no integration events at all**, so no listener ever learned an enrolment happened. It is the message that exposes an intruder with a stolen session enrolling an authenticator of their own, and its absence would have been noticed only by the person it failed to warn. Enrolment now stages `security.mfa.enrolled` in its transaction and a listener sends the notice in the security category, beside the password-changed one. Found by comparing seeded templates with what listeners send: the approved, rejected and cancelled templates looked orphaned too, and are not — the listener builds their code from the instance's status, and cancellation goes through the same announcement. `NotificationTemplateCoverageTests` now fails on a seeded template nothing sends, on a sent template never seeded, and on a request outcome without a template; verified with a one-letter misspelling, which tripped exactly the two directions it should. The same pass found every Platform setting declared and read. |
| 88 | ~~Nothing checked that the portal calls endpoints that exist~~ | — | ✅ **Resolved before it broke anything.** The generated types keep request and response shapes honest but do not bind paths: `callPlatform` takes a string, so a backend route renamed or given another method would leave a portal button answering 404 or 405 with typecheck, lint and every test green. All 110 calls were checked against the contract and all matched. `platform-routes.test.ts` now runs in the Frontend job and fails on a call to a path or method the contract does not publish — verified by breaking one method and one path, both named. Its own first version read only the first of two concatenated path literals and reported a correct call as wrong, which is why it now joins them. **The same pass checked the portal's permission gates**, since a name the Platform never declares hides a screen from every user, administrators included — the failure the inspect permission (#86) shipped on the backend. All ten names were grantable. `PortalPermissionTests` fails the build on one that is not, verified with a one-letter typo. |
| 87 | ~~Five integration events could never reach a subscriber~~ | — | ✅ **Resolved.** Business systems subscribe to event types by webhook, and five of the thirty-five could never be delivered. Two were declared and never constructed: `authz.role.created` and `authz.application.registered`. Three were raised by their aggregates and silently discarded: `authz.role.permission_granted`, `authz.role.permission_revoked` and `identity.user.locked`. `AggregateRoot` said raised events were “called by the unit of work once collected”; **no unit of work collected anything**. Organization had written its own loop and published correctly, which confined the gap to the two modules that trusted the comment — Identity enqueued its other user events by hand and missed the lockout. The audit trail was never affected: it is written directly, not from events. The loop is now `IOutbox.EnqueueRaisedEventsAsync` in the kernel, Organization uses it instead of its own copy, and the five are staged in the transaction that makes the change. `IntegrationEventPublicationTests` fails the build on a declared event nothing constructs and on a module whose aggregates raise events it never stages — verified by removing Identity's staging and by declaring a probe event, both caught; its honest limit, written in the test, is that it proves a module stages *somewhere*, not on every path. Four integration tests assert the outbox rows. **I first reported this as far wider** — every aggregate-raised event dropped, Organization included — from a search whose output `head` had cut off before the Organization results; re-run in full, it was five. **The two gaps this surfaced are closed too.** A subscription naming a type the Platform does not send is now refused with every unknown type named; before, the portal's form was a comma-separated text box, a typo was accepted, and the subscription received nothing for ever. `GET /api/v1/integrations/event-types` lists what exists, the form offers it as a checklist in both languages, and the list is **read from the event records by reflection rather than kept by hand** — a test fails the build unless it equals exactly what the source declares, because a type that stopped being a literal would otherwise drop out and turn every subscription to it into a refusal. |
| 86 | ~~Nobody could check another user's access, the administrator included~~ | — | ✅ **Resolved.** `POST /api/v1/authorization/check` answers a caller's question about themselves with no permission, and about somebody else only with `platform.authorization.inspect` — evaluated in the handler, as a literal. The permission catalogue is derived from endpoint metadata, which is what stops it drifting, so a permission checked inside a handler exists only if the seeder's exception list names it. That list was a private array of literals holding only the delegation permission. **The inspect permission was therefore never created, never granted to the administrator role, and could never be held by anybody**: every question about another user's access was refused, while the authorization guide listed it as working. Nothing tested the other-user path, and the portal does not call it, so nothing noticed. Handlers and the seeder now read one list, `HandlerPermissions`, and the delegation permission's second spelling in the seeder is gone with it. `HandlerPermissionTests` fails the build on a resolver call that names a permission by literal, and on a listed permission nothing checks — the opposite failure, a grant that controls nothing. Verified by restoring the literal and watching both fire. An integration test asserts every handler permission is catalogued. |
| 85 | ~~Seventeen errors were declared and nothing raised them~~ | — | ✅ **Resolved, and the sweep is now a build step.** Listing every `Error` value nothing references found eighteen; one was the step-up defect (#84) and the other seventeen were checked one at a time. **None marked a missing check** — they were rejected designs, written when the shape was imagined one way and left behind when it went another: token issuance answers `InvalidClient` rather than "the application is disabled", so a caller cannot tell a disabled client from a wrong secret; sensitive settings are redacted on read rather than refused; re-declaring a setting updates it instead of conflicting; the dispatcher logs a channel with no provider and stops retrying. Sixteen were deleted. **Leaving them was not harmless**: an error nothing raises is a claim the Platform makes and does not honour, and the next person in one of those handlers may reach for the one sitting right there and restore the design that was rejected, believing they are fixing an oversight. The seventeenth, `PasswordChangeRequired`, stays with its reason — the kernel middleware refuses before any endpoint runs and writes the code itself — and a test keeps the two spellings identical, since a client matching the documented code would otherwise stop recognising the refusal with nothing failing. `ErrorCatalogueTests` fails the build on a new one, refuses a reviewed entry that has started being raised, and refuses to run if it stops seeing the catalogue at all. Verified by reinstating a deleted error and watching exactly one test fail. |
| 84 | ~~An administrator with no second factor was told to confirm their second factor~~ | — | ✅ **Resolved.** Step-up refused with one reason for two situations, so somebody who had never enrolled was told to "verify your second factor and retry" and the portal opened a dialog asking for a code they could not produce. The way out was a screen nothing had sent them to — they would try, fail, and try again. `SecurityErrors.MfaRequiredByPolicy` **had been written for exactly this**, its comment describing the distinction as though it were implemented, and nothing ever raised it. The handler now asks whether there is anything to confirm and refuses accordingly; the response carries `SECURITY.MFA_REQUIRED_BY_POLICY` and the portal says so in both languages instead of offering a dialog. The enrolment is looked up only once the elevation has already failed, so the path everybody takes costs nothing. **Found by listing every `Error` value nothing references** — the same class as `ApplySnakeCaseNames` (#82) and the unraised `SuppliedAssigneesRequired` (#28), which is now three instances of *declared and never called*. 5 unit tests, verified by restoring the old branch and watching exactly the one refusal test fail; the code is spelled in the kernel and in Security, which cannot reference each other, so `StepUpRefusalCodesTests` fails the build if the two spellings drift. **17 more unused `Error` values remain** — most will be innocent, and each is a check that may not be happening. |
| 83 | ~~Nothing checked that the status document agreed with itself~~ | — | ✅ **Resolved, and the guard caught my own error in the commit before it.** The summary sections of this file drifted further than anything else in the repository, because the detail sections get edited every phase and the summaries get edited when somebody remembers: §2 opened with "no capability module has been implemented" while eleven were deployed with screens, §1's summary line said "1 of 22 phases complete" directly beneath a table showing eleven, and Phase 19 read "not started" twenty-seven lines below the header recording the URLs it was serving from. `scripts/check-status-consistency.py` runs in CI and checks the two parts with a shape: the phase summary must count the phase table, and every module in `src/Modules` must have a row. **Its first run failed on the line I had just written** — I counted the phases before changing Phase 19 from not-started to in-progress, so the corrected summary was itself wrong, and had already been pushed. The parser also refuses to run on fewer than twelve phase rows, because a check expressed as "nothing disagreed" passes for ever once the matching breaks. What it cannot check is whether a note beside a phase is still *true*; that has no shape, and stays a person's job. |
...
| 56 | ~~Trace context is not propagated to business applications~~ | — | ✅ **Resolved.** The entry named the gap precisely: both halves were built and the **round trip** was untested. It is now, and from outside — the stub provider records the headers that really crossed the wire, because asserting propagation from inside the Platform would mean the code that might be wrong is the code doing the asserting. A `traceparent` arriving is the `traceparent` leaving: same trace id, **different span id**, which is the half worth pinning — reusing the span id would collapse two operations into one in every viewer. The correlation id comes back unchanged too, which is the half a person uses: a trace id is for a backend, a correlation id is what somebody quotes in a support ticket. It matters because a trace is the only thing that turns "the invoice system says the approval failed" and "the Platform says nothing went wrong" into one timeline. 2 integration tests. |
...
| 65 | Nothing is taking a backup | **High** | The runbook is written and the verification script exists. **Neither has ever run** — there is no backup to verify and, per #66, the script itself has only been syntax-checked. This row used to say "the verification script runs", which read next to #66 is a contradiction, and on the register's highest-severity item it makes the gap look smaller than it is: what exists is a written procedure, not a tested one. Continuous archiving and point-in-time recovery are features of a managed PostgreSQL and the provider is undecided (Q4), so the long-term shape is genuinely blocked. **What is not blocked is a question nobody has answered**: the Platform is deployed on Railway today, and whether that database has provider-level backups switched on is not recorded anywhere in this repository. That is an afternoon's checking and needs no decision about Q4 — and until somebody does it, the honest statement about production is not "backups are deferred" but "nobody knows". Recorded as High because every other risk here is survivable and this one is not: the audit trail cannot be reconstructed from anywhere. |
| 66 | ~~The restore verification has never been run against a real dump~~ | — | ✅ **Resolved, and it could not have passed before.** Reading `scripts/verify-restore.sh` closely enough to wire it into CI found two checks that failed on every good backup: twelve schemas expected against its own list of eleven (ten modules and the kernel), and twelve migration histories where there are eleven. A backup check like that is discovered in the middle of the incident it exists for, and read as “the backup is broken” rather than “the check is”. The end-to-end job now dumps its database after the browser tests — migrated by the real API, seeded with an administrator, just used — and restores it with the runbook's own script, using tools from the postgres:17 image because pg_dump refuses a newer server. **First execution, on every push since:** all ten module schemas and the kernel restored, the audit trail still range-partitioned with its six partitions, a user, three roles and an assignment present, eleven migration histories. **This verifies the procedure, not a backup** — #65 stands: nothing is taking one of production. |
...
| 69 | ~~Test rate limits are raised through a process-global environment variable~~ | — | ✅ **Resolved, and the reason recorded here was wrong.** The limit is now a `UseSetting` on the host, which is per-host, so `RateLimitTests` can drive it down to three without a later class inheriting the raised number. **What this row used to say — and what the comment in `AuthorizationMatrixTests` said — was that test classes run in parallel. They do not**: `AssemblyInfo.cs` disables parallelization outright, and has since Phase 2. The conclusion survived the correction because it never rested on concurrency, only on xUnit not defining the order classes run in; but a justification citing a mechanism the repository has switched off is one somebody eventually acts on, and this is the same documentation drift the register keeps finding — this time **inside the register**. The connection string stays an environment variable and that is not fixable: the composition root resolves it from `builder.Configuration` **before** `builder.Build()`, so a deployment pointed at no database fails at startup rather than at its first request, and nothing a `WebApplicationFactory` contributes is visible that early. That single read is now the entire reason the suite is serialized, said in one place, so the day it changes the attribute comes off in one edit. |
...
| 14 | ~~Employee custom-attribute extension bag not built~~ | — | ✅ **Resolved.** `PUT/GET/DELETE /organization/employees/{id}/attributes`, and the word ARCHITECTURE.md §7.2.2 was doing the work with is **typed**. A free-form JSON column would let an application store anything at all on an employee record — a credential included — and the Platform would have no answer to "what do you hold about this person", which a company is obliged to answer in full. So a key is `<application>.<name>`: namespaced, because a key belonging to nobody makes the first collision silent, with one application overwriting another's value and both convinced they owned it. The database settles a race, not luck. **A credential pasted here is refused** by the check the Configuration module applies to a setting — moved to the kernel, because the argument was never about settings and a second copy of a security heuristic is a second copy that drifts. Fifty per employee, since a bag with no limit is a table somebody eventually uses as a database; replacing one always works at the limit, because a limit that blocked edits would strand an application with fifty attributes it could never correct. Reading returns **everything from every application**, for the same reason the bag is typed. Audited by **key, never by value**: the value may be somebody's medical category, and "who decided this" is the question being asked. This is not a way around the HR boundary — an attribute is opaque to the Platform, and the moment the Platform needs to understand a field it is a Platform field and §4.4 applies. 10 unit tests; the secret guard was verified against a deliberately removed check. **No screen yet**, recorded as #79. |
| 79 | ~~Employee custom attributes have no screen~~ | — | ✅ **Resolved, the same day it was recorded.** A panel on the employees screen: everything kept about one person, by every application, with adding and removing behind `platform.employees.manage` while **reading sits outside that guard** — what is held about somebody is a view permission, and the panel guards its own editing. Each row names the application that owns the key, because reading the namespace off the key is how somebody knows whose note they are looking at. **No formatting, no types, no opinion**: these are an application's own notes and the Platform does not interpret them, so the screen does not pretend to either. The credential refusal is named in full — somebody who has just pasted an API key needs to be told it was not stored and why. 15 catalogue keys at parity. |
