...
| 77 | No automatic re-encryption after an MFA key rotation | Medium | A secret moves to the new key only when the person re-enrols, so the retired key must stay configured indefinitely — and removing it early denies the second factor to everyone who has not re-enrolled since, which is the outage key versioning exists to avoid, reached by a different route. Rewriting each secret on its next successful use would finish a rotation with nobody being asked to do anything; it needs the verification path to be able to save, which it currently cannot. |
