...
| 78 | Outbound subscriptions have no screen | Low | The API registers, reconfigures, suspends, resumes and lists deliveries; the portal does not. A module with an API and no screen is half a module by this project’s own standard, and this is the half that is missing. Recorded the day the API shipped rather than discovered later. |
