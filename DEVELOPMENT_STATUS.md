...
| 76 | A forced password change is enforced by the portal, not by the API | **Medium** | `IdentityErrors.PasswordChangeRequired` is declared and referenced by nothing. The portal reads `mustChangePassword` from `/me` and redirects, which covers everybody who uses a browser — and covers nobody who calls the API directly with a token obtained from the initial password. So a temporary password issued by an administrator is a working credential for the whole API until its holder happens to open the portal. Found while wiring the end-to-end seed, which had to sign in before that change and discovered it could do everything. The fix is a check in the authorization pipeline, exempting the change-password endpoint itself. |
...
| 59 | Flag targeting cannot be edited from the screen | Medium | The toggle sends the flag's existing role and unit ids back unchanged, so a flag can be turned on and off and cannot be aimed. **No longer blocked:** #50 is fixed, so a targeted flag now reaches its target and aiming one is finally useful. What remains is a picker on the configuration screen that knows which roles and units exist. |
